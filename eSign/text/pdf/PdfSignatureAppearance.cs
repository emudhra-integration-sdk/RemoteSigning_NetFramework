...
using eSign.text.error_messages;
using System.Text;
using System.Collections.Generic;
using eSign.text.pdf.security;
using eSign.text.io;
using RDSA;
/*
 * $Id$
...
namespace eSign.text.pdf
{
    ...
    public class PdfSignatureAppearance
    {
        ...
        /** Resizing ValidationMessage in SignatureAppearance */
        private bool _ResizeValidationMessge = false;

        virtual public bool ResizeValidationMessge
        {
            get { return _ResizeValidationMessge; }
            set { _ResizeValidationMessge = value; }
        }
        ...
        */
        ...
        virtual public void SetVisibleSignature(Rectangle pageRect, int page, string fieldName)
        {
            ...
            if (fieldName != null)
            {
                ...
                this.fieldName = fieldName + "-Signature1";
            }
        ...
        public enum RenderingMode
        {
            ...
            /**
             * The rendering mode is name of the signer, image and the description.
             */
            NAME_GRAPHIC_AND_DESCRIPTION,
            /**
             * The rendering mode is just an image.
             */
            GRAPHIC,
            /**
             * The rendering mode gives image on right side and text on left side.
             */
        ...
        virtual public PdfTemplate GetAppearance()
        {
            ...
            if (app[2] == null)
            {
                ...
                if (renderingMode == RenderingMode.NAME_AND_DESCRIPTION ||
                    (renderingMode == RenderingMode.GRAPHIC_AND_DESCRIPTION || renderingMode == RenderingMode.NAME_GRAPHIC_AND_DESCRIPTION && this.SignatureGraphic != null))
                {
                ...
                else if (renderingMode == RenderingMode.GRAPHIC_AND_DESCRIPTION_RTL)
                {
                    if (signatureGraphic == null)
                    {
                        throw new InvalidOperationException(MessageLocalization.GetComposedMessage("a.signature.image.should.be.present.when.rendering.mode.is.GRAPHIC_AND_DESCRIPTION_RTL.only"));
                    }
                    // origin is the bottom-left
                    dataRect = new Rectangle(
                        MARGIN,
                ...
                else if (renderingMode == RenderingMode.NAME_GRAPHIC_AND_DESCRIPTION)
                {
                    //if (signatureGraphic == null)
                    //{
                    //    throw new InvalidOperationException(MessageLocalization.GetComposedMessage("a.signature.image.should.be.present.when.rendering.mode.is.graphic.and.description"));
                    ...
                    //    if (imageScale == 0)
                    //    {
                    //        p.Add(new Chunk(im, signatureRect.Width, signatureRect.Height, false));
                    //    }
                    //    else
                    //    {
                    //        float usableScale = imageScale;
                    //        usableScale = Math.Min((signatureRect.Width - imageScale) / im.Width, (signatureRect.Height - imageScale) / im.Height);

                    //        float w = im.Width * usableScale;
                    //        float h = im.Height * usableScale;
                    //        im.ScaleToFit(w, h);

                    //        x = x + (signatureRect.Width) / 2;
                    //        y = y - (signatureRect.Height) / 2;
                    //        p.Add(new Chunk(im, x, y, false));
                    //    }
                    //}
                    ///*   x = x + (signatureRect.Width - im.ScaledWidth) / 2;
                    //  y = y - (signatureRect.Height - im.ScaledHeight) / 2;
                    //  p.Add(new Chunk(im, x + (signatureRect.Width - im.ScaledWidth) , y, false));
                    // */
                    ...
                    //ct2.RunDirection = runDirection;
                    //ct2.SetSimpleColumn(signatureRect.Left, signatureRect.Bottom, signatureRect.Right, signatureRect.Top, 0, Element.ALIGN_LEFT);
                    //// ct.Go();
                    ...
                    float signedSize = ColumnText.FitText(font, SignerName, sr2, -1, runDirection);
                    ct.SetSimpleColumn(new Phrase(SignerName, font), signatureRect.Left, signatureRect.Bottom, signatureRect.Right, signatureRect.Top, signedSize, Element.ALIGN_LEFT);
                    ct.Go();
            ...
            if (app[4] == null && !acro6Layers)
            {
                ...
                Rectangle sr;
                if (ResizeValidationMessge)
                {
                    t.BoundingBox = new Rectangle(0, rect.Height * (1 - TOP_SECTION), rect.Right / 2, rect.Top);
                    sr = new Rectangle(0, rect.Height * (1 - TOP_SECTION), rect.Right / 2, rect.Top);
                }
                else
                {
                    t.BoundingBox = new Rectangle(0, rect.Height * (1 - TOP_SECTION), rect.Right, rect.Top);
                    sr = new Rectangle(rect.Width - 2 * MARGIN, rect.Height * TOP_SECTION - 2 * MARGIN);
                }
                ...
                if (layer2Font == null)
                    ...
                    font = new Font(Font.FontFamily.HELVETICA, 8, Font.BOLD);
                float size = font.Size;
                ...
                ct.RunDirection = runDirection;
                if (ResizeValidationMessge)
                    ct.SetSimpleColumn(new Phrase(text, font), 0, rect.Height * (1 - TOP_SECTION), rect.Right / 2, rect.Top, size, Element.ALIGN_LEFT);
                else
            ...
            if (frm == null)
            {
                ...
                writer.AddDirectTemplateSimple(frm, new PdfName("FRM"));
                //Signature green tick rescale
                //float scale = Math.Min(rect.Width, rect.Height) * 0.9f;
                float scale = Math.Min(rect.Width, rect.Height) * 0.62f;
                //float x = (rect.Width - scale) / 2;
                float x = (rect.Width - scale) / 1.4f;
                ...
                if (!acro6Layers)
                    ...
                    frm.AddTemplate(app[1], scale, 0, 0, scale, x + 10, y);
        ...
        */
        ...
        virtual public void PreClose(Dictionary<PdfName, int> exclusionSizes)
        {
            ...
            if (fieldExists)
            {
                ...
                //if multiple appreance hard code to 1

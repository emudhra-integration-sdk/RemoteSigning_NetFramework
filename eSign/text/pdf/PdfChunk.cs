...
namespace eSign.text.pdf {
    ...
    public class PdfChunk {
        ...
        internal PdfChunk(Chunk chunk, PdfAction action) {
            ...
            Font f = chunk.Font;
            float size = f.Size;
            if (size == eSign.text.Font.UNDEFINED)
                size = 12;
            baseFont = f.BaseFont;
            //BaseFont bf = f.BaseFont;
            int style = f.Style;
            if (style == eSign.text.Font.UNDEFINED) {
                style = eSign.text.Font.NORMAL;
            }
            if (baseFont == null) {
